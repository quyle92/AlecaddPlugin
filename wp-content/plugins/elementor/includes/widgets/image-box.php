...
class Widget_Image_Box extends Widget_Base {
	...
	protected function _register_controls() {
		...
		$this->start_controls_section(
			'section_style_image_custom',
			[
				'label' => __( 'Custom Img Setting', 'elementor' ),
				'tab'   => Controls_Manager::TAB_STYLE,
			]
		);
		...
		$this->add_responsive_control(
			'image_size',
			[
				'label' => __( 'Width (Custome Img)', 'elementor' ) . ' (%)',
				'type' => Controls_Manager::SLIDER,
				'default' => [
					'size' => 30,
					'unit' => '%',
	...
	protected function render() {
		...
		if ( ! empty( $settings['image']['url'] ) && strpos( $settings['image']['url'],'placeholder.png') !==false ) {
		...
		//var_dump( $settings['link_img']['url'] );die;
		if ( ! empty( $settings['link_img']['url'] ) ) {
				$html .=  '<figure class="elementor-image-box-img"><img src="' . $settings['link_img']['url'] . '"/></figure>';
		}
