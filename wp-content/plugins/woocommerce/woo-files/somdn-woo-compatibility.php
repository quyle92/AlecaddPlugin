...
function somdn_is_user_member_purchase($product_id)
{
	...
	// These will always be true for accounts than can manage the shop
	...
	if (!$can_purchase || !$can_view) {
		$not_restricted = false;
	}

	return apply_filters('somdn_is_user_member_purchase', $not_restricted, $product_id, $can_purchase, $can_view);
}
...
function somdn_is_member_restricted($product_id)
{
	...
	return apply_filters('somdn_is_member_restricted', $is_resticted, $product_id, $rules);
}
